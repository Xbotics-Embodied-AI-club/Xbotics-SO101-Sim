...
# 位移到像素的换算依赖景深（实测增益约 1143 px/m），而画面里不同深度的东西位移量不同，
# 所以定稿值是扫描出来的，见 `experiment_main_v1/scripts/EAI-exp-002/measure_top_offset.py`。
#
# 定稿 (+0.032, +0.028, 0)：三集（ep0/120/250）验证，残差 dy 全归零、dx 在 ±10 px 内，
# 逐像素中位图的互相关从 0.54~0.58 升到 0.77~0.81。剩下的 dx 散布来自各集场景本身不同
# （物体与料箱逐集随机摆），压不下去。
# 相关只在画面**下三成**上算：上半幅是背景墙与随机摆的物体，两侧本来就不同；底座安装板
# 在两侧都有且不动，判别信息全在那里。
TOP_CAMERA_OFFSET = (0.032, 0.028, 0.0)
