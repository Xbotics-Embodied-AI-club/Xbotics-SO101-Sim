...
#    实测（`probe_jaw_chatter.py`，堵转段方向翻转率，次/秒；真机 0.18）。
#    ★ **必须三个场景一起扫。** 早先只扫 cube40 就定了 0.20，套给另两个场景时
#      回归门当场红：cube20 中位 14.32（8 集里 7 集越门）、cylinder40 5.08（6/8）。
#      20mm 方块与 40mm 圆柱的接触几何不同，同一个上限压不住同一个极限环。
#      `recipe.py` 开篇那条「照抄别的场景会整批 0 收」，说的就是这类事。
#      每点 8 集，**成功率全档 8/8，所以判据只剩抖动**：
#        上限    cube40   cube20   cylinder40
#        0.20     1.13     7.29      3.80
#        0.10     0.22     2.55      1.90
#        0.05     0.12     0.37      0.57   ← 取它，三个场景同时触底
#        0.02     0.12     0.38      0.27
#    ★ cube40 那次 40 集扫描里 0.15 曾反弹到 2.40，这次 0.10 就降到 0.22 且不反弹
#      ⇒ 那个反弹是采样噪声，不是拐点。
#
# ★ 取舍要讲明白：2.0 当初是照真机舵机量级取的（STS3215 堵转约 30 kg·cm ≈ 2.9 N·m）。
#   0.20 **离开了数据手册**，是为了修一个求解器假象。判据取「观感更像真机 + 成功率不掉」：
#   真机舵机堵转不嗡嗡响（0.18 次/秒），仿真在 2.0 下每秒来回二十次，在 0.05 下 0.12~0.57 次。
